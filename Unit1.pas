...
type
  ...
  private
      //tableau des chemins de fichiers
        ...
        If (S.Attr And faDirectory)<>0
          ...
          begin
            ...
            begin
              ...
              tabPath.Add(Chemin+S.FindData.cFileName);
...
var
  ...
  saved : bool;
begin
  SaveDialog1.Title:='Selection du Fichier Excel';
  //repertoire de départ 'mes documents'
  SaveDialog1.InitialDir:= GetSpecialFolder('Personal');
  //nom de fichier au depart 'Listedefilms.xls'
  SaveDialog1.FileName:='Listedefilms.xls';
  saved := SaveDialog1.Execute;
  if (saved) then
  begin
    ...
    //aFileName := GetSpecialFolder('Personal')+'\Listedefilms.xls';
...
begin
  if ((listbox1.ItemIndex >=0 ) and (listbox1.itemindex < tabPath.count )) then
    ShellExecute(handle, 'open', PChar( tabPath.Strings[ listbox1.itemindex ]), nil, nil, SW_SHOW);
end;
